...
interface DeezerSearchResult { id: number; title: string; artist: { name: string; }; album: { cover_xl: string | null; }; }
interface ConfirmationMetadata { title: string; artist: string; coverURL: string | null; }
...
export const MusicPage: React.FC = () => {
  ...
  const sortedSongs = useMemo(() => {
    ...
    switch (sortOrder) {
      case 'title_asc': return sortableSongs.sort((a, b) => a.title.localeCompare(b.title));
      case 'artist_asc': return sortableSongs.sort((a, b) => a.artist.localeCompare(b.artist));
  ...
  const handleOpenMetadataModal = (song: Song) => { setSongToUpdate(song); setSearchQuery(sanitizeQuery(song.title)); setModalStep('input'); setIsSearchModalOpen(true); };
  const handleExecuteSearch = async () => { if (!searchQuery) return; setIsSearching(true); const results = await searchTracksOnDeezer(searchQuery); setIsSearching(false); if (results && results.length > 0) { setSearchResults(results); setModalStep('results'); } else { alert(`Nessun risultato trovato per "${searchQuery}".`); } };
  const handleSelectMetadata = (deezerTrack: DeezerSearchResult) => { setSelectedMetadata({ title: deezerTrack.title, artist: deezerTrack.artist.name, coverURL: deezerTrack.album.cover_xl, }); setIsSearchModalOpen(false); setIsConfirmModalOpen(true); };
  const handleConfirmUpdate = async (finalMetadata: ConfirmationMetadata) => { if (!user || !songToUpdate) return; const songRef = doc(db, 'users', user.uid, 'songs', songToUpdate.id); try { await updateDoc(songRef, { title: finalMetadata.title, artist: finalMetadata.artist, coverURL: finalMetadata.coverURL, }); alert(`Metadati per "${songToUpdate.title}" aggiornati con successo!`); } catch (error) { console.error("Errore durante l'aggiornamento del documento:", error); } finally { handleCloseConfirmModal(); } };
  const handleDeleteSong = async (song: Song) => { if (!user) return; if (!window.confirm(`Sei sicuro di voler eliminare "${song.title}"? L'azione è irreversibile.`)) { return; } try { await deleteDoc(doc(db, 'users', user.uid, 'songs', song.id)); const audioFileRef = ref(storage, `music/${user.uid}/${song.fileName}`); await deleteObject(audioFileRef); alert(`"${song.title}" è stato eliminato con successo.`); } catch (error) { console.error("Errore durante l'eliminazione del brano:", error); } };
  ...
  const getStatusText = (status: UploadProgress['status']) => { switch(status) { case 'processing': return 'Inizializzazione...'; case 'uploading': return 'Caricamento...'; case 'complete': return 'Completato!'; case 'error': return 'Errore!'; default: return 'In attesa...'; } };
...
// --- COMPONENTI MODALI (INVARIATI) ---
const MetadataSearchModal = ({ step, query, setQuery, onSearch, isSearching, results, onSelect, onClose }: SearchModalProps) => { return ( <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" onClick={onClose}> <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-md max-h-[80vh] flex flex-col" onClick={e => e.stopPropagation()}> <div className="p-4 border-b dark:border-gray-700 flex justify-between items-center flex-shrink-0"> <h3 className="font-bold text-lg">{step === 'input' ? 'Modifica la Ricerca' : 'Seleziona il Metadato'}</h3> <Button size="icon" variant="ghost" onClick={onClose}><X size={20}/></Button> </div> {step === 'input' ? ( <div className="p-6 space-y-4"> <label htmlFor="search-query" className="text-sm font-medium text-gray-700 dark:text-gray-300">Inserisci i termini di ricerca</label> <input id="search-query" type="text" value={query} onChange={(e) => setQuery(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') onSearch(); }} className="w-full p-2 border rounded-md dark:bg-gray-700 dark:border-gray-600 focus:ring-2 focus:ring-blue-500" placeholder="Es. Queen Bohemian Rhapsody" /> <Button onClick={onSearch} className="w-full" disabled={isSearching}> {isSearching ? (<div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white"></div>) : (<div className="flex items-center justify-center gap-2"><Search size={18} /> Cerca</div>)} </Button> </div> ) : ( <div className="overflow-y-auto"> {results.map(track => ( <div key={track.id} className="p-3 flex items-center gap-4 hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer" onClick={() => onSelect(track)}> <img src={track.album.cover_xl || undefined} alt={`Cover for ${track.title}`} className="w-14 h-14 rounded-md object-cover bg-gray-200 dark:bg-gray-700" /> <div className="flex-1 overflow-hidden"> <p className="font-semibold truncate">{track.title}</p> <p className="text-sm text-gray-500 dark:text-gray-400 truncate">{track.artist.name}</p> </div> </div> ))} </div> )} </div> </div> ); };
const ConfirmMetadataModal = ({ metadata, onConfirm, onClose }: ConfirmModalProps) => { const [title, setTitle] = useState(metadata.title); const [artist, setArtist] = useState(metadata.artist); const handleConfirmClick = () => { onConfirm({ title, artist, coverURL: metadata.coverURL }); }; return ( <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" onClick={onClose}> <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-md" onClick={e => e.stopPropagation()}> <div className="p-4 border-b dark:border-gray-700 flex justify-between items-center"> <h3 className="font-bold text-lg">Conferma Metadati</h3> <Button size="icon" variant="ghost" onClick={onClose}><X size={20}/></Button> </div> <div className="p-6 space-y-4"> <div className="flex justify-center"> <img src={metadata.coverURL || undefined} alt="Cover" className="w-32 h-32 rounded-md object-cover bg-gray-200 dark:bg-gray-700" /> </div> <div> <label htmlFor="title-confirm" className="text-sm font-medium text-gray-700 dark:text-gray-300">Titolo</label> <input id="title-confirm" type="text" value={title} onChange={(e) => setTitle(e.target.value)} className="mt-1 w-full p-2 border rounded-md dark:bg-gray-700 dark:border-gray-600 focus:ring-2 focus:ring-blue-500" /> </div> <div> <label htmlFor="artist-confirm" className="text-sm font-medium text-gray-700 dark:text-gray-300">Artista</label> <input id="artist-confirm" type="text" value={artist} onChange={(e) => setArtist(e.target.value)} className="mt-1 w-full p-2 border rounded-md dark:bg-gray-700 dark:border-gray-600 focus:ring-2 focus:ring-blue-500" /> </div> </div> <div className="p-4 border-t dark:border-gray-700 flex justify-end gap-2"> <Button variant="outline" onClick={onClose}>Annulla</Button> <Button variant="default" onClick={handleConfirmClick}>Salva Modifiche</Button> </div> </div> </div> ); };