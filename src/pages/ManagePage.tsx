...
// FIX: Corretto il percorso di importazione
...
export const ManagePage: React.FC = () => {
  ...
  const openProModal = (feature: string) => { setProFeatureName(feature); setProModalOpen(true); };
  const handleOpenModal = (workout: Workout | null = null) => { setEditingWorkout(workout); setModalOpen(true); };
  const handleSaveWorkout = async (data: WorkoutData) => { try { if (editingWorkout) { await updateWorkout(editingWorkout.id, data); } else { await addWorkout({ ...data, createdAt: new Date(), history: [] }); } setModalOpen(false); } catch (error) { console.error("Errore nel salvataggio della scheda:", error); alert("Si è verificato un errore."); } };
  const handleImportClick = () => { fileInputRef.current?.click(); };
  const handleFileImport = async (event: React.ChangeEvent<HTMLInputElement>) => { const file = event.target.files?.[0]; if (!file) return; const reader = new FileReader(); reader.onload = async (e) => { try { const text = e.target?.result; if (typeof text !== 'string') { throw new Error("Impossibile leggere il file."); } const data = JSON.parse(text); const isValidWorkout = (item: any): item is Omit<WorkoutData, 'createdAt' | 'history'> => item && typeof item.name === 'string' && Array.isArray(item.exercises); let workoutsToImport: Omit<WorkoutData, 'createdAt' | 'history'>[] = []; if (Array.isArray(data)) { workoutsToImport = data.filter(isValidWorkout); } else if (isValidWorkout(data)) { workoutsToImport.push(data); } if (workoutsToImport.length > 0) { await Promise.all(workoutsToImport.map(workout => addWorkout({ ...workout, createdAt: new Date(), history: [] }))); alert(`${workoutsToImport.length} scheda/e importata/e con successo!`); } else { throw new Error("Il file JSON non contiene schede valide."); } } catch (error: any) { console.error("Errore durante l'importazione:", error); alert(`Impossibile importare il file: ${error.message}`); } }; reader.readAsText(file); if(event.target) event.target.value = ''; };
  const jsonExample = `{ "name": "Giorno A - Spinta", "exercises": [ { "name": "Panca Piana", "sets": 4, "reps": "8-10" } ] }`;
    ...
    <div className="container mx-auto p-4 space-y-6">
      ...
      <div>
        <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
          <div><h1 className="text-3xl font-bold">Gestisci Schede</h1><p className="text-gray-500 dark:text-gray-400">Crea, modifica e importa i tuoi piani.</p></div>
        </header>
        <Card className="mb-4">
          <h3 className="font-semibold mb-2 flex items-center gap-2"><Info size={18} /> Importa Scheda da File</h3>
          <p className="text-sm text-gray-600 dark:text-gray-400 mb-2">Puoi importare una singola scheda o un elenco di schede. Assicurati che il file <code>.json</code> abbia la seguente struttura:</p>
          ...
          <div className="flex gap-2 w-full mt-4">
            ...
            <Button onClick={handleImportClick} variant="secondary" className="flex-1"><Upload size={16} /> Importa File</Button>
            <Button onClick={() => openProModal('Generazione Scheda')} className={`flex-1 text-white ${activeTheme.bgClass}`}><Sparkles size={16} /> Crea con AI</Button>
          </div>
        </Card>
            ...
            {workouts.map(workout => (
              <Card key={workout.id} className="flex flex-col justify-between">
                ...
                <div className="flex items-center justify-between mt-4 gap-2">
                   <Button onClick={() => setActiveWorkout(workout.id)} variant="secondary" className={`flex-1 ${activeWorkout?.id === workout.id ? activeTheme.bgClass + ' text-white' : ''}`}><CheckCircle size={16}/> {activeWorkout?.id === workout.id ? 'Attiva' : 'Seleziona'}</Button>
                   <div className="flex">
                      <button onClick={() => openProModal(`Ottimizzazione "${workout.name}"`)} className="p-2 text-gray-500 hover:text-purple-500" title="Ottimizza con AI"><Sparkles size={18} /></button>
